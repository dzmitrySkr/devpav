...
function Wrapper() {
  ...
  let storeTasks = useSelector((state) => state.reducer.tasks);
  ...
  //Из стор таска передаем локальную переменную для поиска
  useEffect(() => {
    setActiveTasks(storeTasks);
  }, [storeTasks]);

  //поиск по тегу
  let tegSearch = (e) => {
    setSearchInp(e);
    let a = storeTasks.filter((item) => {
      return item.text
        .toLowerCase()
        .includes(`${e ? "#" + e : e}`.toLowerCase());
    });
    setActiveTasks(a);
  };
